...
var Event = function ($) {
  return function (properties) {
    ...
    this.props.start_datetime = properties.start_datetime;
	  var venuenow = properties.venue.substring(properties.venue.indexOf('.')+1)
    this.props.address = venuenow;
    ...
    this.render_event = function (distance, zipcode) {
      ...
      var rendered = $("<div class=montserrat/>").addClass('event-item ' + that.className).html('\n            <div class="event-item lato ' + that.className + '" lat="' + lat + '" lon="' + lon + '">\n              <h5 class="time-info">\n                <span class="time-info-dist">' + (distance ? distance + "mi&nbsp;&bull;&nbsp;" + datetime : datetime) + '</span><p id="local" style="color:#000;display:block;clear:both;float:none;"></p></h5>\n              <h3>\n                <a target="_blank" href="' + that.props.url + '?source=map">' + that.props.title + '</a>\n              </h3>\n              <span class="label-icon"></span>\n              \n              <p>' + that.props.address + '</p>\n              <div>\n                <a class="rsvp-link" href="' + that.props.url + '?source=map" target="_blank">RSVP</a>\n              </div>\n            </div>\n            ');
...
var MapManager = function ($, d3, leaflet) {
  return function (eventData, campaignOffices, zipcodes, options) {
    ...
    var accessToken = "pk.eyJ1Ijoia2VubmV0aHBlbm5pbmd0b24iLCJhIjoiY2l6bmJ3MmFiMDMzZTMzbDJtdGxkM3hveSJ9.w4iOGaL2vrIvETimSXUXsw";
    ...
    var initialize = function initialize() {
      ...
      uniqueLocs.forEach(function (item) {
        ...
        //     .addTo(overlays);
        // }
        if (item.className == 'action') {
          L.circleMarker(item.latLng, { radius: 5, className: item.className, color: 'white', fillColor: '#ef3030', opacity: 0.8, fillOpacity: 0.7, weight: 2 }).on('click', function (e) {
            _popupEvents(e);
          }).addTo(overlays);
        } else {
    ...
    */
    ...
    module.filterByCoords = function (coords, distance, sort, filterTypes) {
      ...
      function addhighlightedMarker(lat, lon) {
        var highlightedMarker = new L.circleMarker([lat, lon], { radius: 5, color: '#ef4134', fillColor: '#537882', opacity: 0.8, fillOpacity: 0.7, weight: 2 }).addTo(centralMap);
